...
if __name__ == "__main__":
    ...
    Dis_src_opt = torch.optim.Adam(Dis_src.parameters(), lr=args.lr)
    Dis_tar_opt = torch.optim.Adam(Dis_tar.parameters(), lr=args.lr)
    Gen_src_opt = torch.optim.Adam(Gen_src.parameters(), lr=args.lr)
    Gen_tar_opt = torch.optim.Adam(Gen_tar.parameters(), lr=args.lr)
...
for epoch in range(args.n_epoch):
    for i, (a_real, b_real) in enumerate(zip(src_loader, tar_loader)):
        ...
        if (i + 1) % 10 == 0:
            logging.info("Epoch: (%3d) (%5d/%5d)" % (epoch, i + 1, min(len(src_loader), len(tar_loader))))
        ...
        if (i + 1) % 50 == 0:
            with torch.no_grad():
                Gen_src.eval()
                Gen_tar.eval()
                a_real_test = Variable(iter(src_test_loader).next()[0])
                b_real_test = Variable(iter(tar_test_loader).next()[0])
                ...
                vutils.save_image(pic, '%s/Epoch_(%d)_(%dof%d).jpg' % (save_dir, epoch, i + 1, min(len(src_test_loader), len(tar_test_loader))), nrow=3)